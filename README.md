...
MemOnDemand is a Python package and CLI that addresses these problems with three
coordinated mechanisms: a collection-dependent multi-level hierarchy, dual distilled
and detailed memory at every level, and on-demand promotion under a bounded active-state
budget. Compact records route each query; selected source-resolved details support the
answer; observed use updates which nodes remain easy to reuse.
...
| Model gateway | One `general` chat and embedding contract configured through environment variables. |
...
MemOnDemand separates the retrieval system into three operational planes:
...
Promotion and evidence loading are deliberately separate. Promotion changes which nodes
remain easy to reuse across queries; it does not bypass the answer-context budget or
turn distilled text into answer evidence.
...
```bash
git clone git@github.com:Hik289/MemOnDemand.git
cd MemOnDemand
...
```text
memondemand/
  core/       general API adapter, environment loading, config templates
