...
class TokenLedger:
    ...
    def add(self, n_in: int, n_out: int) -> None:
        self.total_in += n_in
        ...
        self.total_cost_usd += (
            n_in * input_price + n_out * output_price
        ) / 1_000_000
        self.n_calls += 1
...
def embed_texts(texts: List[str], model_name: str = EMBED_MODEL) -> np.ndarray:
    ...
    if backend == "general":
        ...
        log.info("Encoding %d texts through the general embedding API ...", len(texts))
        t0 = time.time()
        vecs = embed_batch(list(texts), batch_size=128)
        log.info("Embedded %d texts in %.1fs (general)", len(texts), time.time() - t0)
...
def build_dynamic_hierarchy(
    ...
    while True:
        ...
        while True:
            ...
            decision = call_general_depth(depth_prompt, ledger, dry_run=dry_run)
