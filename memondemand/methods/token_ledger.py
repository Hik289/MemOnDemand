...
class TokenLedger:
    ...
    def _compute_cost(self, model_alias: str, in_t: int, out_t: int) -> float:
        prices = self.prices.get(model_alias) or self.prices.get(
            "general", {"input": 0.0, "output": 0.0}
        )
...
def _self_test() -> int:
    ...
    led.record("distilled_text_gen", "general", 100, 30, 0.5)
    led.record("distilled_text_gen", "general", 120, 28, 0.6)
    led.record("final_answer", "general", 1500, 200, 2.0)
    g = led.grand_total()
    print("grand:", g)
