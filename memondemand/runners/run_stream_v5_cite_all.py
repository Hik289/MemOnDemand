...
class Embedder:
    def __init__(self, model_name: str = EMBED_MODEL):
        ...
        # Optional query embedding cache to avoid repeated hosted API calls.
        self._query_cache = {}
        ...
        if backend == "general":
            from memondemand.data.general_embedder import GeneralEmbedder
            self.dim = int(_os.environ.get("MEMONDEMAND_EMBED_DIM", "1536"))
            log.info("loading general API embedder (dim=%d) ...", self.dim)
            self._general = GeneralEmbedder(dim=self.dim)
            self.model = None
        else:
    ...
    def encode(self, texts: Sequence[str], **kwargs) -> np.ndarray:
        ...
        if self._query_cache:
            ...
            if misses:
                if self.backend == "general":
                    miss_vecs = self._general.encode(misses, **kwargs).astype(np.float32)
                else:
        ...
        # No cache configured
        if self.backend == "general":
            return self._general.encode(texts_list, **kwargs).astype(np.float32)
        params = {
            "batch_size": 64,
...
def _bm25_then_dense_rerank(ctx, query_text, k=12, wide_k=50):
    ...
    if l0_idx is None or len(l0_idx.ids) == 0:
        ...
        return bm_hits[:k]
    # 3. Embed query; the optional cache is backend independent.
...
def cost_for(alias: str, in_t: int, out_t: int) -> float:
    del alias
    input_price = float(os.environ.get("MEMONDEMAND_API_INPUT_COST_PER_MILLION", "0"))
    output_price = float(os.environ.get("MEMONDEMAND_API_OUTPUT_COST_PER_MILLION", "0"))
    return (in_t * input_price + out_t * output_price) / 1_000_000
