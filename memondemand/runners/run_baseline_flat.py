...
from memondemand.core.api_adapter import call as api_call, estimate_cost

logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(message)s", datefmt="%H:%M:%S")
log = logging.getLogger(__name__)

def call_general(prompt, max_tokens=512):
    result = api_call(
        "general",
    ...
    usage = result["usage"]
    return result["text"], usage["input_tokens"], usage["output_tokens"]
...
def main():
    ...
    with open(answers_path, "a") as fout:
        for _, row in df.iterrows():
            ...
            t0 = time.time()
            ans, n_in, n_out = call_general(prompt)
    ...
    summary = {
        ...
        "cost_usd": round(
            sum(
                estimate_cost(
                    os.environ.get("MEMONDEMAND_API_MODEL", "general"),
                    a["tokens_answer_in"],
                    a["tokens_answer_out"],
                )
                for a in answers
