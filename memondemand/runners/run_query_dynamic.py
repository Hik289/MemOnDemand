...
def call_general(prompt: str, max_tokens: int = 512) -> Tuple[str, int, int]:
...
def run_single_query(
    ...
    for step in range(max_nav_steps):
        ...
        t0 = time.time()
        try:
            nav_text, nav_in, nav_out = call_general(nav_prompt, max_tokens=128)
    ...
    try:
        promo_text, promo_in, promo_out = call_general(promo_prompt, max_tokens=256)
    ...
    try:
        answer_text, ans_in, ans_out = call_general(answer_prompt, max_tokens=512)
    except Exception as e:
