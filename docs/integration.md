...
MemOnDemand exposes one `general` model-gateway contract for chat and embeddings:
...
Keep tenancy, provider routing, and account-level deployment policy at the gateway
layer. MemOnDemand configuration is limited to endpoint, key, model, and runtime
behavior.
